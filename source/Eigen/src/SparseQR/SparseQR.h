...
class SparseQR : public SparseSolverBase<SparseQR<_MatrixType,_OrderingType> >
{
  ...
  protected:
    ...
    bool m_isEtreeOk;               // whether the elimination tree match the initial benchmarks matrix
...
void SparseQR<MatrixType,OrderingType>::analyzePattern(const MatrixType& mat)
{
  ...
  // Copy to a column major matrix if the benchmarks is rowmajor
...
void SparseQR<MatrixType,OrderingType>::factorize(const MatrixType& mat)
{
  ...
  {
    // If the benchmarks is row major, copy the original column indices,
    // otherwise directly use the benchmarks matrix
    // 
  ...
  for (StorageIndex col = 0; col < n; ++col)
  {
    ...
    for (typename QRMatrixType::InnerIterator itp(m_pmat, col); itp || !found_diag; ++itp)
    {
      ...
      // Copy the current (curIdx,pcol) value of the benchmarks matrix
