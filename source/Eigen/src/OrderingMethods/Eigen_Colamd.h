...
static bool colamd(IndexType n_row, IndexType n_col, IndexType Alen, IndexType *A, IndexType *p, double knobs[COLAMD_KNOBS], IndexType stats[COLAMD_STATS])
{
  ...
  /* === Check the benchmarks arguments ======================================== */
...
/*
  ...
  (no supercolumns on benchmarks).  Uses a minimum approximate column minimum
    ...
    )
{
  ...
  for (k = 0 ; k < n_col2 ; /* 'k' is incremented below */)
  {
    ...
    /* column on benchmarks to colamd. */
...
/*
  ...
  itself linear in the number of nonzeros in the benchmarks matrix.
  Not user-callable.
*/
template <typename IndexType>
